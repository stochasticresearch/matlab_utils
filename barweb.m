...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
        bw_legend
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',legendTextSize, 'AutoUpdate', 'off');
		legend boxoff;
	else
		handles.legend = [];
	end
...
%     end
    % changed below to work w/ higher versions of matlab.  This code was
    % provided by Flo in the forums on the website:
